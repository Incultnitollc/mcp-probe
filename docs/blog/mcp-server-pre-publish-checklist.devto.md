...
### Distribution
10. **Install metadata ships** — clear package name, runnable example, fresh README, and a `server.json` so the official MCP Registry can discover you. Devs find tools at install-time, not search-time.
