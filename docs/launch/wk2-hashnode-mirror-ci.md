# Wk2 CI article — Hashnode mirror paste payload

**Source article:** `docs/ci.md` ("MCP Server CI Pipeline (GitHub Actions): from lint to publish, with a conformance gate")
**Wk2 query slot:** Q6 (greenfield) · P2.1 in pre-launch playbook
**Target publish window:** within 24h of `docs/ci.md` landing on `main`

---

## Manual paste steps (sequence)

1. **Confirm `docs/ci.md` is on `main`** — Hashnode canonical_url points at it; the file must exist at the canonical URL before publishing the mirror.
   ```bash
   curl -sIo /dev/null -w '%{http_code}\n' \
     https://github.com/Incultnitollc/mcp-probe/blob/main/docs/ci.md
   # Expect: 200
   ```
2. **Sign in** at https://hashnode.com under the Incultnito account. If no account exists yet, sign up with `incultnito@gmail.com` and reserve the handle `incultnitollc` (matches the dev.to account naming).
3. **New post:** click the "+" → "Write a post" (or visit https://hashnode.com/draft).
4. **Title field** — paste exactly:
   ```
   MCP Server CI Pipeline (GitHub Actions): from lint to publish, with a conformance gate
   ```
5. **Subtitle / description** — paste exactly:
   ```
   A working GitHub Actions pipeline for a TypeScript MCP server — lint, type-check, unit tests, an MCP conformance gate, and tag-driven publish with OIDC provenance. Copy-pasteable YAML, with every job explained and the common pitfalls called out.
   ```
6. **Body** — paste the markdown block in the section below (**Paste payload — body**). This is the article body with YAML frontmatter stripped (Hashnode uses its own metadata UI, not frontmatter).
7. **Cover image** — see "Cover image guidance" below. Upload via the cover-image icon in the Hashnode editor.
8. **Tags** — add exactly these five, in this order:
   - `mcp`
   - `githubactions`
   - `cicd`
   - `typescript`
   - `devops`
9. **Canonical URL** — open "Article Settings" in the right sidebar → "SEO" or "Original Article URL" field. Paste:
   ```
   https://github.com/Incultnitollc/mcp-probe/blob/main/docs/ci.md
   ```
   This is the SEO-critical step. Without it, Hashnode will index as the canonical and split rank from the GitHub source.
10. **Slug** — Hashnode auto-generates from the title. Override to:
    ```
    mcp-server-ci-pipeline-github-actions
    ```
11. **Publish** — click "Publish". Do not schedule; ship immediately so the canonical URL takes effect.
12. **Verify publish** — open the published URL in incognito, confirm:
    - Title renders correctly
    - Code blocks are syntax-highlighted (Hashnode auto-detects from triple-backtick `yaml` / `json` / `bash`)
    - Canonical URL in `<head>` points at the GitHub file (view-source → search `canonical`)
    - All five tags appear at the bottom
13. **Post-publish:** append the Hashnode URL to `docs/citation-log.md` under a "Wk2 Q6 — CI article distribution" heading (alongside the GitHub URL and any future dev.to mirror).

---

## Cover image guidance

**Filename to upload:** `og-card-ci.png` (do not reuse `og-card.png` from the descriptions series — different article surface, different cover)

**Concept (for separate image-gen task, NOT in this commit):**
- Background: off-white (`#FAFAFA`) — matches Incultnito brand
- Foreground: monospace text block in `#820855` (brand magenta) showing five gate labels stacked vertically:
  ```
  lint
  typecheck
  test
  ...
  publish
  ```
- Right-aligned: subtle GitHub Actions check-mark glyph (✓) repeated five times, off-white-on-magenta
- Title overlay (bottom-left, Inter or similar): "MCP Server CI Pipeline"
- No photography, no people, no AI-rendered scenes. Editorial flat. Cult-luxe minimal.
- Dimensions: 1600×840 (Hashnode recommended; same ratio as OG card standard)

**Fallback** if no time to generate: upload the existing `docs/assets/og-card.png` — carries brand voice consistency across the series even though it's not CI-specific.

---

## Pre-publish checklist

- [ ] `docs/ci.md` committed to `main` and visible at canonical URL (HTTP 200)
- [ ] Word count of the body matches `docs/ci.md` (within 20 words — Hashnode strips frontmatter only)
- [ ] All inline links resolve (especially `docs/ci-example.md` — converted to absolute URL in the paste payload below, since Hashnode won't resolve repo-relative links)
- [ ] Canonical URL set BEFORE publish click (Hashnode caches the canonical at publish time; editing post-publish doesn't always propagate to crawlers)
- [ ] No banned words in body: shop now, AI playground, credit packs, marketplace (consumer sense), invest, profit
- [ ] Tags are lowercase, no spaces (Hashnode is strict on tag format)
...
For more conformance-gate patterns — HTML reports, latency benchmarks, remote HTTP servers, GitLab CI, CircleCI, pre-commit hooks — see [the CI examples reference](https://github.com/Incultnitollc/mcp-probe/blob/main/docs/ci-example.md). That doc is the reference for the gate itself; this one is the pipeline around it.
...
- **LLM-side behavior.** Whether a model picks your tool over a competitor's, whether the description disambiguates — those are behaviors, not properties, and need a different evaluation loop.
...
## Post-publish: append to citation log

After Hashnode publish, append a line to `docs/citation-log.md`:

```markdown
### Wk2 Q6 — CI article distribution

- 2026-05-15 — GitHub canonical: https://github.com/Incultnitollc/mcp-probe/blob/main/docs/ci.md
- 2026-05-15 — Hashnode mirror: [PASTE-HASHNODE-URL-AFTER-PUBLISH]
```

## Distribution cadence note

- **No Reddit self-link** from `u/incultnito` (per `feedback_reddit_velocity.md` — wait until Wk4+ for any self-link)
- **GitHub Discussions:** OK to add a fresh comment on `Incultnitollc/mcp-probe#11` linking the Hashnode mirror, framed as "CI pipeline writeup for the conformance gate side of the project — companion to the descriptions series"
- **MCP Discord:** DO NOT post (per `reference_mcp_official_channels.md` — Contributor Discord forbids product marketing)
- **dev.to mirror:** optional second mirror, can ship 24–48h after Hashnode if Δ-target needs a boost
